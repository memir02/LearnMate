...
export default function CreateQuestionPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert(error.response?.data?.message || 'Soru oluşturulurken bir hata oluştu.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 text-gray-900">
      <h1 className="text-3xl font-bold mb-6">Yeni Soru Oluştur</h1>
      ...
      <form onSubmit={handleSubmit} className="space-y-6 bg-white rounded-lg shadow p-6">
        ...
        <div className="flex gap-4">
          ...
          >
            {loading ? 'Oluşturuluyor...' : 'Soru Oluştur'}
          </button>
          <button
            type="button"
            onClick={() => router.back()}
