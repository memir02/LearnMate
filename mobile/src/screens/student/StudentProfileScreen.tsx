...
export default function StudentProfileScreen() {
  ...
  const [parentPhone, setParentPhone] = useState(user?.student?.parentPhone ?? '');
  const [grade, setGrade] = useState(user?.student?.grade ?? '');
  ...
  const handleCancelEdit = () => {
    ...
    setParentPhone(user?.student?.parentPhone ?? '');
  ...
  const initials = `${user?.student?.firstName?.[0] ?? ''}${user?.student?.lastName?.[0] ?? ''}`.toUpperCase();
  const gradeLabel = grade ? `${grade}. Sınıf` : 'Belirtilmemiş';
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.avatarSection}>
          ...
          <View style={styles.roleBadge}>
            <Text style={styles.roleText}>🎓 Öğrenci · {gradeLabel}</Text>
          </View>
            ...
            <View style={styles.fieldGroup}>
              ...
              <View style={styles.fieldRow}>
                <View style={[styles.field, { flex: 1 }]}>
                  ...
                  {isEditing ? (
                    ...
                    <Text style={styles.fieldValue}>{user?.student?.firstName}</Text>
                  )}
                </View>
                <View style={[styles.field, { flex: 1 }]}>
                  ...
                  {isEditing ? (
                    ...
                    <Text style={styles.fieldValue}>{user?.student?.lastName}</Text>
                  )}
                </View>
              </View>
              ...
              <View style={styles.field}>
                ...
                {isEditing ? (
                  ...
                  <Text style={styles.fieldValue}>{gradeLabel}</Text>
                )}
              </View>
              ...
              <View style={styles.field}>
                ...
                {isEditing ? (
                  ...
                  <Text style={styles.fieldValue}>{user?.student?.phone || 'Belirtilmemiş'}</Text>
                )}
              </View>

              {/* Veli Telefonu */}
              <View style={styles.field}>
                <Text style={styles.fieldLabel}>Veli Telefonu</Text>
                {isEditing ? (
                  <TextInput
                    style={styles.input}
                    value={parentPhone}
                  ...
                  <Text style={styles.fieldValue}>{user?.student?.parentPhone || 'Belirtilmemiş'}</Text>
                )}
              </View>
...
const styles = StyleSheet.create({
  ...
  avatar: {
    ...
    backgroundColor: '#f0fdf4', borderWidth: 3, borderColor: Colors.success,
    alignItems: 'center', justifyContent: 'center', marginBottom: 4,
  },
  initials: { fontSize: 32, fontWeight: '800', color: Colors.success },
  fullName: { fontSize: 22, fontWeight: '800', color: Colors.textPrimary },
  roleBadge: {
    paddingHorizontal: 12, paddingVertical: 4, borderRadius: 20,
    backgroundColor: '#f0fdf4',
  },
  roleText: { fontSize: 12, color: Colors.success, fontWeight: '700' },
  ...
  gradeChips: { flexDirection: 'row', gap: 8, paddingVertical: 4 },
  gradeChip: {
    width: 40, height: 40, borderRadius: 10, borderWidth: 1.5,
    borderColor: Colors.border, backgroundColor: Colors.white,
    alignItems: 'center', justifyContent: 'center',
  },
  gradeChipSelected: { backgroundColor: Colors.primaryLight, borderColor: Colors.primary },
  gradeChipText: { fontSize: 13, fontWeight: '700', color: Colors.textSecondary },
  gradeChipTextSelected: { color: Colors.primary },
